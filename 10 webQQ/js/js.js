...
* @Last Modified time: 2016-08-17 15:05:59
*/
...
window.onload=function(){
	...
	var oInfo=document.getElementById('information');
	...
	function bodyImg(){
		if(getCookie('bgImg')){
			...
			document.body.style.backgroundImage='url(images/bg23.jpg)';	
		}
	...
	function userFn(){
		...
		oUser.onmouseover=function(ev){
			oUl.style.display='block';
			oUl.style.background='#fff';
			oUl.style.color='#000';
			oUl.style.zIndex=1000;
		}	
	...
	function insFn(){
		var n=8;
		var oA=oInfo.getElementsByTagName('a')[0];
		...
		setTimeout(function(){
			startMove(oInfo,{'right':-600,'width':600},'hc',function(){
				oA.style.display='block';
				oInfo.onOff=true;
			});
			clearInterval(timer);
			oSpan.innerHTML='';
			oSpan.style.backgroundImage='url(images/login_x.png)';
			oSpan.onOff=true;
			drag(oInfo,function(){
				oInfo.onOff=false;
			});
		...
		oInfo.onmouseover=function(){
			if(oInfo.onOff){
				startMove(oInfo,{'right':0},'hc');
			}
		}
		oInfo.onmouseout=function(){
			if(oInfo.onOff){
				startMove(oInfo,{'right':-600},'hc',function(){
					oInfo.onOff=true;
				});
			}
		}
